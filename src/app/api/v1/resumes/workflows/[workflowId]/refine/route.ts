...
export async function POST(request: Request, { params }: Params) {
  ...
  try {
    return await withMutationGuards(request, async () => {
      ...
      const input = await parseJsonBody(request, refineResumeInputSchema);
      return jsonOk(await (await getRuntime()).services.customerResumes.refine(ctx, workflowId, input), { status: 202 });
    });
  } catch (error) {
