...
export async function GET(request: Request) {
  ...
  try {
    ...
    const alerts = runtime.services.radar.listAlerts(ctx).map(toAlertView);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const runtime = await getRuntime();
    const created = runtime.services.radar.createAlert(ctx, {
      name: body.name,
      query: body.query,
    ...
    if (body.active === false) {
      runtime.services.radar.updateAlert(ctx, created.publicId, { enabled: false });
