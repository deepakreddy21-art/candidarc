...
export { mapApplicationToUi, mapBackendStageToUi } from "./mappers/application-ui";
  ...
  if (mode === "postgres") {
    ...
    if (!ownedEvidence.length) {
      await repos.evidence.create({
        ...
        result: "Reduced deployment time by 40% using Python and Kubernetes.",
        technologies: ["Python", "TypeScript", "Kubernetes"],
        confidence: "high",
        sourceType: "employment",
        claimText:
          "Platform Engineer, January 2024 – Present. Reduced deployment time by 40% using Python and Kubernetes.",
        ...
        payload: { source: "demo-seed", metrics: ["40% deployment time reduction"] },
      });
      await repos.evidence.create({
        id: newId("ev"),
        publicId: "ev-demo-education",
        tenantId: demo.tenantId,
        ownerUserId: demo.userId,
        candidateProfileId: null,
