...
/** Paths exempt from CSRF — keep in sync with csrf-route-inventory.test.ts */
...
export function assertCsrf(request: Request): void {
  ...
  if (!MUTATIONS.has(request.method.toUpperCase()) || CSRF_EXEMPT_PATHS.some((item) => path.startsWith(item))) return;
