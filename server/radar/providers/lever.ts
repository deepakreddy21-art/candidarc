...
export class LeverProvider implements JobSourceProvider {
  ...
  async fetchBoard(input: BoardFetchInput): Promise<JobSourceResult> {
    if (shouldFetchLiveBoard("lever")) return fetchPublicBoard("lever", input);
  ...
  async fetchListing(input: ListingFetchInput): Promise<JobSourceListing | null> {
    if (shouldFetchLiveBoard("lever")) return fetchPublicListing("lever", input);
  ...
  async verifyListing(input: ListingVerificationInput): Promise<JobVerificationResult> {
    if (shouldFetchLiveBoard("lever")) {
      try {
        const listing = await fetchPublicListing("lever", input);
  ...
  async healthCheck(): Promise<ProviderHealth> {
    if (shouldFetchLiveBoard("lever")) return { ok: true, enabled: this.enabled, message: "Live public-board adapter configured; connectivity is checked when a board is fetched", checkedAt: new Date().toISOString() };
    return {
      ok: true,
